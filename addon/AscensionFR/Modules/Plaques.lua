...
-- ----------------------------------------------------------------------------
-- DRAGONUI GÈRE-T-IL LES BARRES DE VIE ? (24/07/2026, signalement joueur :
-- « les buffs/DoT au-dessus des barres de vie apparaissent une seconde puis
-- disparaissent ». Corrigé en coupant NOS plaques -> c'était bien nous.)
--
-- DragonUI identifie la créature d'une barre de vie en LISANT le nom affiché
-- (nameplates/pipeline/discovery.lua:136, GetPlateName -> ogNameText:GetText)
-- puis en le comparant au nom du jeu :
--     StripRealm(UnitName(unit)) == plateName   (identity.lua:51, gather:234)
-- Il relit ce nom EN CONTINU (à chaque changement de cible/survol). Quand on
-- le traduit, « Sanglier » ne vaut jamais « Boar » : DragonUI perd l'unité et
-- masque toutes ses auras (auras.lua:1023, host:Hide()). D'où le clignotement
-- exact : plaque affichée en anglais -> auras OK -> notre veilleuse traduit
-- (0,25 s) -> DragonUI ne reconnaît plus -> auras coupées.
--
-- On ne peut PAS coopérer : les fonctions de DragonUI vivent dans une table
-- privée (`local addon = select(2, ...)`), jamais exposée globalement — aucun
-- moyen d'accrocher sa lecture du nom. Le seul remède robuste : quand
-- DragonUI est là, on ne traduit pas les noms de barres de vie. DragonUI
-- affiche les siens (en anglais) et ses auras fonctionnent. Les joueurs SANS
-- DragonUI gardent le français, rien ne change pour eux.
--
-- Échappatoire pour qui aurait DÉSACTIVÉ le module barres de vie de DragonUI
-- et voudrait quand même nos noms français :
--   /run AscensionFRSaved.Options.plaquesMalgreDragonUI = true
local function GereeParDragonUI()
    if Options() and Options().plaquesMalgreDragonUI then return false end
    return IsAddOnLoaded and IsAddOnLoaded("DragonUI") and true or false
end

-- Les barres de vie ne se traduisent que si notre module est actif ET que
-- DragonUI ne les pilote pas.
local function PlaquesActives()
    return Actif() and not GereeParDragonUI()
end
...
-- MICRO-FREEZES Linux/Proton (bloc 11, 28/07/2026 — signalement précis :
-- un à-coup toutes les ~20 s, fortement réduit en coupant les plaques).
-- Deux coûts rabotés SANS changer le comportement :
--   1. plus AUCUNE allocation par tick : « { WorldFrame:GetChildren() } »
--      fabriquait une table de TOUS les enfants (des centaines) à chaque
--      nouvelle plaque — en combat, c'est en continu, et c'est le ramasse-
--      miettes de Lua qui rend la monnaie par à-coups périodiques (bien
--      plus sensibles sous Wine/Proton). Les varargs sont parcourus
--      directement, zéro table.
--   (La repasse des visibles RESTE à 0,25 s : le banc verifier_plaques a
--   mordu quand j'ai tenté de la ralentir — une plaque déjà à l'écran
--   compte sur elle, pas seulement sur OnShow. Le vrai coût était
--   l'allocation, pas la comparaison de chaînes.)
local function AccrocherNouveaux(...)
    for i = 1, select("#", ...) do
        local cadre = select(i, ...)
        if not connues[cadre] then
            ...
            pcall(cadre.HookScript, cadre, "OnShow", function(c)
                if PlaquesActives() then pcall(RepeindreCadre, c, 2) end
            end)
        end
    end
end
...
veilleuse:SetScript("OnUpdate", function(self, ecoule)
    ...
    depuis = 0
    -- PlaquesActives (et pas seulement Actif) : dès que DragonUI est chargé,
    -- on cesse de traduire les barres de vie — sinon on casse ses auras.
    if not PlaquesActives() then return end
    local n = WorldFrame:GetNumChildren()
    if n ~= nb_connus then
        nb_connus = n
        AccrocherNouveaux(WorldFrame:GetChildren())
    end

...
--
-- CORRIGÉ au bloc 3 (28/07/2026). La comparaison LITTÉRALE rendait
-- invisibles 3 520 modèles sur 47 620 : quand le modèle COMMENCE par une
-- variable (« $s1% chance to… »), la ligne affichée commence par le nombre
-- calculé (« 15% chance to… ») et les 12 premiers caractères ne coïncident
-- JAMAIS — l'échec n'était pas journalisé, le modèle périmé restait anglais
-- à vie sans qu'aucun rapport ne le mentionne (mesuré à l'audit du 28/07).
-- On NORMALISE donc les deux côtés avant de comparer : variables $ du
-- modèle et nombres calculés de l'affichage deviennent le même « # ». La
-- propriété anti-bruit tient : une ligne de stats française ne ressemble
-- toujours pas à un modèle anglais, normalisé ou pas.
local function NormaliserPourModele(texte)
    texte = SansCouleurs(texte or "")
    -- les variables du modèle : $s1, $12345s1, $d, $t1, $/1000;s1…
    texte = string.gsub(texte, "%$[%w%./;]+", "#")
    -- les nombres calculés de l'affichage (2, 2.5, 2,5)
    texte = string.gsub(texte, "%d+[%.,]?%d*", "#")
    -- deux jetons voisins ou des espaces multiples n'en font qu'un
...
local function CorpsAuModele(corps, sorts)
    local debut = string.sub(NormaliserPourModele(corps), 1, 12)
    if debut == "" then return false end
    for _, sid in ipairs(sorts) do
        local s = AFR.DB.Sorts[tonumber(sid)]
        if s then
            if s.DE and string.sub(NormaliserPourModele(s.DE), 1, 12)
                == debut then
                return true
            end
            if s.DE2 and string.sub(NormaliserPourModele(s.DE2), 1, 12)
                == debut then
                return true
            end
        end
...
-- Une ligne est du BRUIT pour la récolte/le journal si :
--  - c'est une DURÉE de buff dynamique (« X remaining / restantes ») : jamais
--    dans la base telle quelle, et déjà gérée par TraduireDuree ;
--  - elle est DÉJÀ FRANÇAISE : l'octet de tête 0xC3 (« \195 ») marque tous
--    les accents FR (é è ê à â ç ï…), absents de l'anglais. Sûr et rapide en
--    Lua 5.1 (motifs sur octets). Constat du 24/07 (fichier de Dan : des buffs
--    français et des durées journalisés « non alignés »).
local function EstBruit(t)
    if type(t) ~= "string" or t == "" then return true end
    if string.find(t, "remaining") or string.find(t, "restante") then
        return true
    end
    if string.find(t, "\195") then return true end   -- accents FR
    return false
end

-- Une VRAIE description anglaise à récolter : longue, non bruitée.
local function DescARecolter(texte)
    if texte and string.len(texte) > 20 and not EstBruit(texte) then
        return texte
    end
    return nil
end

local function TraiterSort(tooltip, id, nomEN, discret)
    if not id then return end
    local s = AFR.DB.Sorts[id]
    if s then
        AFR.TraduireInfobulleSort(tooltip, id, discret)
        -- ANGLE MORT bouché le 20/07 (constat de Dan) : une entrée
        -- INCOMPLÈTE — nom français, description absente — n'était NI
        -- récoltée (id connu) NI journalisée. On la complète ici. Mais PAS
        -- au passage discret (rattrapage OnShow : re-récolterait), PAS si la
        -- couche communautaire la sert déjà, et seulement une VRAIE
        -- description anglaise (plus de durées récoltées, cf. « Cozy Fire »
        -- récolté avec « 22 seconds remaining » pour description).
        if not s.D and nomEN and not discret
            and not (AFR.DB.SortsLignes and AFR.DB.SortsLignes[id]) then
            local nb = tooltip:NumLines()
            local desc = nb > 1 and LigneGauche(tooltip, nb)
            local texte = DescARecolter(desc and desc:GetText())
            if texte then
                AFR.Recolter("Sorts", id, { N = nomEN, D = texte })
            end
        end
    elseif nomEN and not discret then
        -- Sort inconnu de la base : on le récolte (le nom vaut d'être appris),
        -- mais sans description bruitée (durée, ligne déjà française).
        local nb = tooltip:NumLines()
        local desc = nb > 1 and LigneGauche(tooltip, nb)
        AFR.Recolter("Sorts", id,
            { N = nomEN, D = DescARecolter(desc and desc:GetText()) })
    end
end
...
local function SurSort(tooltip)
    ...
    if id and AFR.BulleCompteurs then
        AFR.BulleCompteurs.sort = AFR.BulleCompteurs.sort + 1
    end
...
-- Branchements
-- ----------------------------------------------------------------------------
GameTooltip:HookScript("OnTooltipSetItem", MESURER("Bulle:objet", SurObjet))
GameTooltip:HookScript("OnTooltipSetSpell", MESURER("Bulle:sort", SurSort))
GameTooltip:HookScript("OnTooltipSetUnit", MESURER("Bulle:unite", SurUnite))
-- Bulles de TALENTS CoA (CATalentBaseMixin, lu dans patch-B le 24/07) : le
-- client compose par SetSpellByID — notre OnTooltipSetSpell passe — PUIS il
-- ajoute encore des lignes (bloc d'aura incrusté, « Rank 0/1 ») et appelle
-- Show(). On repasse donc sur l'état FINAL — au Show() EXPLICITE
-- (hooksecurefunc : OnShow seul peut tomber AVANT l'ajout du bloc, quand la
-- bulle devient visible dès SetSpellByID). Réservé aux sorts à description
-- connue : ni récolte ni journal ici (drapeau discret). Verrou : notre
-- propre re-Show de mise en page ne nous rappelle pas en boucle.
AFR.BulleCompteurs = { sort = 0, show = 0 }
local rattrapage_en_cours = false
local function RattraperBulle(tooltip)
    if rattrapage_en_cours or not AFR.Actif() then return end
    local ok, nomEN, _, id = pcall(tooltip.GetSpell, tooltip)
    if not ok then return end
    local s = id and AFR.DB.Sorts[id]
    if s and s.D then
        AFR.BulleCompteurs.show = AFR.BulleCompteurs.show + 1
        rattrapage_en_cours = true
        pcall(TraiterSort, tooltip, id, nomEN, true)
        rattrapage_en_cours = false
    end
end
hooksecurefunc(GameTooltip, "Show", MESURER("Bulle:rattrape", RattraperBulle))
GameTooltip:HookScript("OnShow", RattraperBulle)
...
hooksecurefunc(GameTooltip, "Show", MESURER("Bulle:repasse", RepasseStatique))
if ItemRefTooltip then

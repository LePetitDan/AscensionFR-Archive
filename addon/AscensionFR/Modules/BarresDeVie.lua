...
-- les addons de nameplates se disputent. Vécu deux fois (<joueur>, puis <joueur>) :
...
-- ZÉRO ALLOCATION PAR TIC (programme 10, bloc 4, 01/08/2026).
--
-- C'est le remède déjà appliqué à Plaques.lua le 28/07 pour les micro-freezes
-- Linux/Proton, et qui était resté à faire ici. « { WorldFrame:GetChildren() } »
-- fabriquait une table de TOUS les enfants du monde — des centaines en ville —
-- à chaque fois que le compte bougeait, c'est-à-dire en continu dès qu'il y a
-- du monde autour. Les varargs se parcourent directement : aucune table.
--
-- Ce que ça vaut, mesuré (outils/mesurer_ramassage.py) : sur 600 enfants,
-- l'allocation coûtait 0,038 ms par tic contre 0,014 ms sans — mais surtout
-- elle produisait 3,8 Mo de déchet par minute. Ce déchet n'est pas payé sur le
-- tic qui le crée : il est payé plus tard, d'un coup, par le ramassage. C'est
-- la facture différée qui fait l'à-coup.
local function AccrocherNouveaux(...)
    for i = 1, select("#", ...) do
        local enfant = select(i, ...)
...
-- L'instrument de mesure (/afr perf) a besoin de savoir à qui appartient ce
-- cadre : sans la bibliothèque `debug` de Lua, que WoW retire, une fonction ne
-- peut pas dire de quel fichier elle vient.
if AFR.Perf then AFR.Perf.Suivre("BarresDeVie", frame) end

-- ----------------------------------------------------------------------------
-- JALON « Plaques » (programme 10, 01/08/2026).
--
-- Plaques.lua est le suspect n° 1 des lags, et c'est précisément pour ça qu'on
-- n'a PAS le droit d'y toucher tant qu'on n'a pas de chiffres. Il ne peut donc
-- pas déclarer son cadre comme le module ci-dessus vient de le faire.
--
-- On procède par encadrement : ce jalon-ci ouvre la portée « Plaques », et
-- celui posé en tête de Metiers.lua la referme. Dans le .toc, le SEUL fichier
-- chargé entre les deux est Modules\Plaques.lua — tout cadre à OnUpdate apparu
-- dans cet intervalle vient donc de lui. C'est déterministe, et ça ne touche
-- pas une ligne de chez lui.
--
-- Si l'ordre du .toc change un jour, le raisonnement tombe : ces deux jalons
-- doivent rester collés de part et d'autre de Modules\Plaques.lua.
-- ----------------------------------------------------------------------------
if AFR.Perf then AFR.Perf.Jalon("Plaques") end

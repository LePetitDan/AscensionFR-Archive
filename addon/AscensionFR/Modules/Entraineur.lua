...
local function ConstruireIndex()
    index_desc = {}
    local function poser(cle, fiche)
        local deja = index_desc[cle]
        if deja == nil then
            index_desc[cle] = fiche
        elseif deja and deja.N ~= fiche.N then
    ...
    for _, s in pairs(AFR.DB.Sorts) do
        if type(s) == "table" and s.N then
            if s.DE then
                for _, variante in ipairs(Variantes(s.DE)) do
                    poser(CleFloue(variante), s)
                end
            end
            -- SECONDS MODÈLES (bloc 5, 28/07/2026) : les corrections posent
            -- des couples D2/DE2 (aura) que l'index ignorait — 861 talents
            -- CoA sur 3 928 restaient « hors index » au banc verifier_coa
            -- alors que leur français était en base. Même neutralisation
            -- des doublons au nom divergent.
            if s.DE2 and s.D2 then
                local fiche2 = { N = s.N, D = s.D2, DE = s.DE2 }
                for _, variante in ipairs(Variantes(s.DE2)) do
                    poser(CleFloue(variante), fiche2)
                end
            end
        end
    end

...
-- Referme la portée « Plaques » ouverte à la fin de BarresDeVie.lua : tout
-- cadre à OnUpdate apparu depuis vient de Modules\Plaques.lua, seul fichier
-- chargé entre les deux (voir Perf.lua). Ne pas déplacer sans revoir le .toc.
if AFR.Perf then AFR.Perf.Jalon("Metiers") end
...
if AFR.Perf then AFR.Perf.Suivre("Metiers", cadence) end
cadence:Hide()
local TRANCHE = 250
...
if AFR.Perf then AFR.Perf.Suivre("Metiers", minuteur) end
minuteur:Hide()
local ecoule = 0

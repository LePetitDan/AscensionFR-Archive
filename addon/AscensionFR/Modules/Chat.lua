...
-- ----------------------------------------------------------------------------
-- Messages SYSTÈME de quêtes (bloc 10, 28/07/2026 — remonté deux fois par
-- les joueurs) : « Quest accepted: Bones for Grork » arrivait tel quel dans
-- le chat alors que le journal est en français. Le texte est COMPOSÉ à
-- l'affichage (gabarit anglais + titre) : on convertit le gabarit du CLIENT
-- en motif, on recompose avec le gabarit français officiel (DB.UI), et le
-- TITRE passe par le pont des quêtes. Aucune globale n'est touchée — pur
-- filtre d'affichage, la méthode sûre de ce module.
-- ----------------------------------------------------------------------------
local GABARITS_SYSTEME = {
    "ERR_QUEST_ACCEPTED_S", "ERR_QUEST_COMPLETE_S", "ERR_QUEST_FAILED_S",
    "ERR_QUEST_OBJECTIVE_COMPLETE_S",
}
local reglesSysteme

local function EnMotifSysteme(gabarit)
...
local function ConstruireReglesSysteme()
    reglesSysteme = {}
    for _, cle in ipairs(GABARITS_SYSTEME) do
        local en = _G[cle]
        ...
        if type(en) == "string" and en ~= "" and type(fr) == "string"
            ...
            table.insert(reglesSysteme,
                { motif = EnMotifSysteme(en), fr = fr })
        end
    end
end

local function FiltreSysteme(self, event, message, ...)
    if not AFR.Actif() or type(message) ~= "string" then return false end
    if not reglesSysteme then ConstruireReglesSysteme() end
    for _, r in ipairs(reglesSysteme) do
        local titre = string.match(message, r.motif)
        if titre then
            local fr_titre = (AFR.TitreQueteFrancais
                and AFR.TitreQueteFrancais(titre)) or titre
            return false, string.format(r.fr, fr_titre), ...
        end
    end
    return false
end

ChatFrame_AddMessageEventFilter("CHAT_MSG_SYSTEM", FiltreSysteme)
